...
input_disc_noise = tflearn.input_data(
    shape=[None, z_dim], name='input_disc_noise')
input_disc_real = tflearn.input_data(
    shape=[None, 28, 28, 1], name='input_disc_real')
...
"""
y_disc_fake=np.zeros([total_samples])
y_disc_real=np.ones([total_samples])
...
for i in range(total_samples):
    y_disc_fake.append(y_disc_fake[0])
    y_disc_real.append(y_disc_real[0])
    pass
y_disc_fake = np.asfarray(y_disc_fake)
y_disc_real = np.asfarray(y_disc_real)
...
y_gen = [[1, 0]]
for i in range(total_samples):
    y_gen.append(y_gen[0])
    pass
y_gen = np.asfarray(y_gen)
...
for i in range(10):
    ...
    for j in range(4):
        ...
        img = np.reshape(
            np.repeat(g[j][:, :, np.newaxis], 3, axis=2), newshape=(28, 28, 3))
        a[j][i].imshow(img)
